...
async function run() {
  ...
  const sqlPath = path.join(
    ...
    "prisma",
    "migrations",
    "20260212_ia_prompt_remove_content",
    "migration.sql"
  );
  ...
  try {
    ...
    console.log("IA prompt content removal migration applied successfully.");
  } finally {
    client.release();
    await pool.end();
...
run().catch((error) => {
  console.error("IA prompt content removal migration failed:", error);
  process.exit(1);
});
