console.log("Deprecated: IA migration already applied.");
process.exit(0);
...
async function run() {
  ...
  try {
    ...
    console.log("IA migration applied successfully.");
  } finally {
    client.release();
    await pool.end();
...
run().catch((error) => {
  console.error("IA migration failed:", error);
  process.exit(1);
});
